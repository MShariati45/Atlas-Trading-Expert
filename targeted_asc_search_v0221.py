...
selected=[];used=set()
xs=[x for x in raw if x['family']=='ASCENDING_TRIANGLE' and datetime.fromisoformat(x['time']).year>=2018]
...
for fam,t in selected:
 ...
 if r.get('events'): print(fam,t.isoformat(),'eligible',r.get('eligible_bars'),'events',r.get('events'))
(ROOT/'targeted_ascending_triangle_windows_v0221.json').write_text(json.dumps(results,indent=2))
print('windows',len(results),'with events',sum(bool(r.get('events')) for r in results))
