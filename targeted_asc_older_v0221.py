...
selected=[];used=set()
xs=[x for x in raw if x['family']=='ASCENDING_TRIANGLE' and datetime(2017,1,1,tzinfo=timezone.utc)<=datetime.fromisoformat(x['time'])<datetime(2020,1,1,tzinfo=timezone.utc)]
for x in reversed(xs):
 day=x['time'][:10]
 if day in used: continue
 selected.append(('ASCENDING_TRIANGLE',datetime.fromisoformat(x['time'])));used.add(day)
 if len(selected)>=40: break
results=[]
for fam,t in selected:
 r=run_window(t);r['target_family']=fam;results.append(r)
 es=[e for e in r.get('events',[]) if e.get('pattern_type')=='ASCENDING_TRIANGLE']
 if es: print('FOUND',t.isoformat(),r.get('eligible_bars'),es)
(ROOT/'targeted_ascending_triangle_older_windows_v0221.json').write_text(json.dumps(results,indent=2))
print('windows',len(results),'asc events',sum(any(e.get('pattern_type')=='ASCENDING_TRIANGLE' for e in r.get('events',[])) for r in results))
