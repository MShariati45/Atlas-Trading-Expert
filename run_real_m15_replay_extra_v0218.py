...
END=datetime(2020,10,16,0,0,tzinfo=timezone.utc)
...
(ROOT/'extra_real_m15_replay_summary_v0218.json').write_text(json.dumps(summary,indent=2),encoding='utf-8')
with (ROOT/'extra_real_m15_actionable_events_v0218.csv').open('w',newline='') as f:
    w=csv.DictWriter(f,fieldnames=['time','agent','status','event_id','entry','final_stop','pkg_state']); w.writeheader(); w.writerows(event_rows)
with (ROOT/'extra_real_m15_primary_packages_v0218.csv').open('w',newline='') as f:
